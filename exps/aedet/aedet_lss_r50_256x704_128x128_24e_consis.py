...
from models.aedet_consis import AeDet
...
backbone_conf = {
    ...
    'z_bound_pooling_consis': z_bound_pooling_consis,    
    'use_cdn': True,
...
class AeDetLightningModel(LightningModule):
    ...
    def __init__(self,
                 ...
                 swap_pers_uv = False,
                 swap_bev_xy = False,
                 pooling_consistency = False,
                 pooling_consistency_v2 = False,
                 pooling_consistency_v2_z_bin_size = 5,
                 z_bound_pooling_consis_size = 0.1,
                 pool_pa = False,
                 pool_ba = False,
                 pool_pa_mlp = False,
                 pool_ba_mlp = False,
                 pool_pers_detach = False,
                 pool_bev_detach = False,
                 pool_loss = False,
                 pool_loss_coef = 1.0,
                 **kwargs):
        super().__init__()
        ...
        if pooling_consistency_v2:
            self.backbone_conf['z_bound_pooling_consis'][2] = z_bound_pooling_consis_size
            z_bound_pooling_consis[2] = z_bound_pooling_consis_size
        if bevdepth:
        ...
        self.backbone_conf['use_da_origin'] = use_da_origin
        self.backbone_conf['use_da_prob'] = use_da_prob
        ...
        self.consistency_cfg=dict(
            ...
            pooling_consistency = pooling_consistency,
            pooling_consistency_v2 = pooling_consistency_v2,
            pooling_consistency_v2_z_bin_size = pooling_consistency_v2_z_bin_size, 
            z_bound_pooling_consis = z_bound_pooling_consis,
            x_bound = x_bound,
            pool_pa = pool_pa,
            pool_ba = pool_ba,
            pool_pa_mlp = pool_pa_mlp,
            pool_ba_mlp = pool_ba_mlp,
            pool_pers_detach = pool_pers_detach,
            pool_bev_detach = pool_bev_detach,
            pool_loss = pool_loss,
            pool_loss_coef = pool_loss_coef,
        )
        
        self.depth_gt_training = depth_gt_training
        ...
        self.model = AeDet(self.backbone_conf,
                              ...
                              depth_gt_training = self.depth_gt_training,
                              )
        self.mode = 'valid'
    ...
    def forward(self, sweep_imgs, mats, depth_labels = None):
        return self.model(sweep_imgs, mats, depth_labels=depth_labels)
    ...
    def training_step(self, batch):
        ...
        preds, depth_preds, img_feats, bev_feats, bev_neck, feature_map_pooling_consis = self(sweep_imgs, mats, depth_labels)
        if isinstance(self.model, torch.nn.parallel.DistributedDataParallel):
            targets = self.model.module.get_targets(gt_boxes, gt_labels)
            loss_heatmap, loss_bbox, loss_consis = self.model.module.loss(targets, preds, img_feats, bev_feats, bev_neck, mats, self.backbone_conf, self.consistency_cfg, feature_map_pooling_consis)
        else:
            targets = self.model.get_targets(gt_boxes, gt_labels)
            loss_heatmap, loss_bbox, loss_consis = self.model.loss(targets, preds, img_feats, bev_feats, bev_neck, mats, self.backbone_conf, self.consistency_cfg, feature_map_pooling_consis)
    ...
    def eval_step(self, batch, batch_idx, prefix: str):
        ...
        if torch.cuda.is_available():
            ...
            sweep_imgs = sweep_imgs.cuda()
        if self.depth_gt_training:
            preds = self.model(sweep_imgs, mats, depth_labels.cuda())
        else:
    ...
    def val_dataloader(self):
        ...
        from functools import partial
        val_return_depth = self.depth_gt_training
        ...
        val_loader = torch.utils.data.DataLoader(
            ...
            shuffle=False,
            collate_fn=partial(collate_fn, 
                        is_return_depth=val_return_depth),
            num_workers=4,
            sampler=None,
        )
        return val_loader
...
def run_cli():
    ...
    parent_parser.add_argument('--amp_backend',  default='native', type=str)
    ...
    parent_parser.add_argument('--use_da_origin', action='store_true',)
    parent_parser.add_argument('--use_da_prob', action='store_true',)
    parent_parser.add_argument('--use_pa', action='store_true',)
    parent_parser.add_argument('--use_pa_1x1', action='store_true',)
    parent_parser.add_argument('--use_pa_gelu', action='store_true',)
    parent_parser.add_argument('--use_pa_2layer', action='store_true',)
    parent_parser.add_argument('--use_ba', action='store_true',)
    parent_parser.add_argument('--use_ba_1x1', action='store_true',)
    parent_parser.add_argument('--use_ba_gelu', action='store_true',)
    parent_parser.add_argument('--use_ba_2layer', action='store_true',)
    parent_parser.add_argument('--depth_gt_training', action='store_true',)
    parent_parser.add_argument('--swap_pers_uv', action='store_true',)
    parent_parser.add_argument('--swap_bev_xy', action='store_true',)
    parent_parser.add_argument('--pooling_consistency', action='store_true',)
    parent_parser.add_argument('--pooling_consistency_v2', action='store_true',)
    parent_parser.add_argument('--pooling_consistency_v2_z_bin_size', default=10, type=int)
    parent_parser.add_argument('--z_bound_pooling_consis_size', default=0.1, type=float)
    parent_parser.add_argument('--pool_pa', action='store_true',)
    parent_parser.add_argument('--pool_ba', action='store_true',)
    parent_parser.add_argument('--pool_pa_mlp', action='store_true',)
    parent_parser.add_argument('--pool_ba_mlp', action='store_true',)
    parent_parser.add_argument('--pool_pers_detach', action='store_true',)
    parent_parser.add_argument('--pool_bev_detach', action='store_true',)
    parent_parser.add_argument('--pool_loss', default="nega_cos", type=str) # ['nega_cos', 'infonce', 'l1']
    parent_parser.add_argument('--pool_loss_coef', default=1.0, type=float)
