...
from dataset.nusc_mv_det_dataset_with_pers import NuscMVDetDatasetwithPers, collate_fn
from evaluators.det_mv_evaluators import DetMVNuscEvaluator
from models.aedet_fcos import AeDetFcos
...
# pipeline=[{'type': 'LoadImageFromFileMono3D'}, {'type': 'LoadAnnotations3D', 'with_bbox': True, 'with_label': True, 'with_attr_label': True, 'with_bbox_3d': True, 'with_label_3d': True, 'with_bbox_depth': True}, {'type': 'Resize', 'img_scale': (1600, 900), 'keep_ratio': True}, {'type': 'RandomFlip3D', 'flip_ratio_bev_horizontal': 0.5}, {'type': 'Normalize', 'mean': [103.53, 116.28, 123.675], 'std': [1.0, 1.0, 1.0], 'to_rgb': False}, {'type': 'Pad', 'size_divisor': 32}, {'type': 'DefaultFormatBundle3D', 'class_names': ['car', 'truck', 'trailer', 'bus', 'construction_vehicle', 'bicycle', 'motorcycle', 'pedestrian', 'traffic_cone', 'barrier']}, {'type': 'Collect3D', 'keys': ['img', 'gt_bboxes', 'gt_labels', 'attr_labels', 'gt_bboxes_3d', 'gt_labels_3d', 'centers2d', 'depths']}]
...
# )
# load_anno_3d = LoadAnnotations3D()
...
class AeDetLightningModel(LightningModule):
    ...
    def __init__(self,
        ...
        self.model = AeDetFcos(self.backbone_conf,
                              self.head_conf,
                              is_train_depth=True)
    ...
    def training_step(self, batch):
        (sweep_imgs, mats, timestamps_batch, img_metas, gt_boxes, gt_labels, pers_gt_labels, depth_labels) = batch
        ...
        input_list = [mats, pers_gt_labels]
        preds, depth_preds, fcos_loss = self(sweep_imgs, input_list)
        ...
        self.log('fcos_loss', fcos_loss)
        return detection_loss + depth_loss #+ fcos_loss
    ...
    def train_dataloader(self):
        train_dataset = NuscMVDetDatasetwithPers(
    ...
    def val_dataloader(self):
        val_dataset = NuscMVDetDatasetwithPers(
            ida_aug_conf=self.ida_aug_conf,
            bda_aug_conf=self.bda_aug_conf,
            classes=self.class_names,
            data_root=self.data_root,
...
def run_cli():
    ...
    parser.set_defaults(profiler='simple',
                        ...
                        precision=16,
                        default_root_dir='./outputs/aedet_lss_r50_256x704_128x128_24e_7split_idax')
