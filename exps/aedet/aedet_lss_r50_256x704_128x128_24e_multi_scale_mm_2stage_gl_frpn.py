...
from models.aedet_mm_multi_scale_2stage_gl_frpn import AeDet
...
class AeDetLightningModel(LightningModule):
    ...
    def __init__(self,
                 ...
                 overfit=False,
                 use_4split=False,
                 deform_conv_lstm = False,
                 convlstm_layer = 3,
                 motion_gate = False,
                 key=2,
                 layer_norm=False,
                 bevdepth=False,
                 mm_no_residual=False,
                 **kwargs):
        super().__init__()
        ...
        self.backbone_conf['bevdepth'] = False
        ...
        if bevdepth:
            self.backbone_conf['bevdepth'] = True
        ...
        self.backbone_conf['deform_conv_lstm'] = deform_conv_lstm
        self.backbone_conf['motion_gate'] = motion_gate
        self.backbone_conf['convlstm_layer'] = convlstm_layer
        self.backbone_conf['num_keys'] = len(self.key_idxes) + 1
        self.backbone_conf['mm_no_residual'] = mm_no_residual
        
        self.head_conf['bev_backbone_conf']['in_channels'] = backbone_conf['output_channels'] * (
            1)
        ...
        self.use_4split = use_4split # chgd
        self.num_sweeps = 1
    ...
    def training_step(self, batch):
        ...
        if isinstance(self.model, torch.nn.parallel.DistributedDataParallel):
            ...
            mats['heatmap_gt_1'] = targets[0]
            mats['heatmap_gt_2'] = targets[4]
            mats['heatmap_gt_3'] = targets[5]
            mats['heatmap_gt_4'] = targets[6]
            
            preds, depth_preds, heatmap_losses = self(sweep_imgs, mats)
            detection_loss = self.model.module.loss(targets[:4], preds)
            #first_detection_loss = self.model.module.loss(targets, first_preds)
        else:
            ...
            mats['heatmap_gt_4'] = targets[6]
            preds, depth_preds, heatmap_losses = self(sweep_imgs, mats)
            detection_loss = self.model.loss(targets[:4], preds)
            #first_detection_loss = self.model.loss(targets, first_preds)
        ...
        self.log('heatmap_ce_loss', heatmap_losses[0])
        self.log('heatmap_dice_loss', heatmap_losses[1])
        return detection_loss + depth_loss #+ first_detection_loss
...
def run_cli():
    ...
    parent_parser.add_argument('--key', default = 2, type=int)
    parent_parser.add_argument('--layer_norm', action='store_true')
    parent_parser.add_argument('--convlstm_layer', default = 2, type=int)
    parent_parser.add_argument('--deform_conv_lstm', default=True)
    parent_parser.add_argument('--motion_gate', default=True)
    parent_parser.add_argument('--bevdepth', action='store_true')
    parent_parser.add_argument('--mm_no_residual', action='store_true')
    ...
    parser.set_defaults(profiler='simple',
                        ...
                        precision=16,
                        default_root_dir='./outputs/aedet_lss_r50_256x704_128x128_24e_multi_scale_mm_2stage_gl_frpn')
