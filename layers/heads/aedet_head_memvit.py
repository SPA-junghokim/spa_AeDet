...
class AeDetHead(CenterHead):
    ...
    def get_bev_mask_loss(self, gt_bev_mask, pred_bev_mask):
        pred_bev_mask = pred_bev_mask.float()
    ...
    def forward(self, x):
        ...
        ret_values = super().forward(fpn_output) # chgd
        return ret_values
    ...
    def get_targets(self, gt_bboxes_3d, gt_labels_3d):
        heatmaps1, anno_boxes, inds, masks = multi_apply(
        ...
        return heatmaps1, anno_boxes, inds, masks
    ...
    def loss(self, targets, preds_dicts, **kwargs):
        ...
        return_mask_ce_loss = 0
        return_mask_dice_loss = 0
        gt_heatmap_1 = torch.max(torch.cat(heatmaps, dim=1), dim=1)[0] > 0
        mask_ce_loss, mask_dice_loss = self.get_bev_mask_loss(gt_heatmap_1.float(), preds_dicts[0][0]['heatmap1'].squeeze(1).float())
        return_mask_ce_loss += mask_ce_loss
        return_mask_dice_loss += mask_dice_loss
        ...
        return return_loss, return_mask_ce_loss, return_mask_dice_loss