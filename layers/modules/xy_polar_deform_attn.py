...
class XY_POLAR_BEV_DEFORM_ATTN(BaseModule):
    def __init__(self,
                 embed_dims=80,
                 num_heads=8,
                 num_points=4,
        ...
        self.query3x3 = nn.Conv2d(embed_dims, embed_dims, kernel_size=3, stride=1, padding=1)
        ...
        self.value_proj3x3 = nn.Conv2d(embed_dims, embed_dims, kernel_size=3, stride=1, padding=1)
    ...
    def get_reference_points(self, H, W,  device='cuda', dtype=torch.float):
        ...
        ref_2d = ref_2d.repeat(1, 1, 1).unsqueeze(2)
        return ref_2d
    ...
    def forward(self,
        ...
        if identity is None:identity = query.view(bs, -1, num_query).permute(0,2,1).contiguous()
