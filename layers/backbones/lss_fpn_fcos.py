...
class LSSFPN(nn.Module):
    def __init__(self, x_bound, y_bound, z_bound, d_bound, final_dim,
        ...
        self.conv512 = nn.Conv2d(512,256,kernel_size=(1,1),stride=1)
        self.conv1024 = nn.Conv2d(1024,256,kernel_size=(1,1),stride=1)
        self.conv2048 = nn.Conv2d(2048, 256,kernel_size=(1,1),stride=1)
    ...
    def get_cam_feats(self, imgs):
        ...
        backbone_res = self.img_backbone(imgs)
        
        img_feats = self.img_neck(backbone_res)[0]
        
        # chgd
        backbone_res = list(backbone_res)
        for idx in range(len(backbone_res)-1):
            if idx == 0:
                backbone_res[idx] = backbone_res[idx].reshape(-1, 6, \
                    backbone_res[idx].shape[1], backbone_res[idx].shape[2], backbone_res[idx].shape[3])
            
            idx += 1
            if idx == 1:
                backbone_res[idx] = self.conv512(backbone_res[idx])
                backbone_res[idx] = backbone_res[idx].reshape(-1, 6, \
                    backbone_res[idx].shape[1], backbone_res[idx].shape[2], backbone_res[idx].shape[3])
            elif idx == 2:
                backbone_res[idx] = self.conv1024(backbone_res[idx])
                backbone_res[idx] = backbone_res[idx].reshape(-1, 6, \
                    backbone_res[idx].shape[1], backbone_res[idx].shape[2], backbone_res[idx].shape[3])
            else:
                backbone_res[idx] = self.conv2048(backbone_res[idx])
                backbone_res[idx] = backbone_res[idx].reshape(-1, 6, \
                    backbone_res[idx].shape[1], backbone_res[idx].shape[2], backbone_res[idx].shape[3])

        #backbone_res = tuple(backbone_res)
            
        img_feats = img_feats.reshape(batch_size, num_sweeps, num_cams,
                                      img_feats.shape[1], img_feats.shape[2],
                                      img_feats.shape[3])
        return img_feats, backbone_res
    ...
    def _forward_single_sweep(self,
        ...
        img_feats, backbone_feats = self.get_cam_feats(sweep_imgs)
        source_features = img_feats[:, 0, ...] # (B, 6, 512, 16, 44)
        ...
        if is_return_depth:
            return feature_map.contiguous(), depth, backbone_feats # chgd
        return feature_map.contiguous(), backbone_feats
