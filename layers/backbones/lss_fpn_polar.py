...
class PersAggregation(nn.Module):
    """
    pixel cloud feature extraction
    """

    def __init__(self, in_channels, mid_channels, out_channels):
        super(PersAggregation, self).__init__()
...
class LSSFPN(nn.Module):
    def __init__(self, x_bound, y_bound, r_bound, a_bound, z_bound, d_bound, final_dim,
                 downsample_factor, output_channels, img_backbone_conf,
                 img_neck_conf, depth_net_conf, use_cdn=False,
                 virtual_depth_bins=None, min_focal_length=None, min_ida_scale=None, use_da=False, use_pa=False,
                 xy_bev_head_with_deform_attn = False):
        """Modified from `https://github.com/nv-tlabs/lift-splat-shoot`.

        Args:
            r_bound (list): Boundaries for x.
            a_bound (list): Boundaries for y.
        ...
        self.register_buffer(
            'voxel_size',
            torch.Tensor([row[2] for row in [r_bound, a_bound, z_bound]]))
        self.register_buffer(
            'voxel_coord',
            torch.Tensor([
                row[0] + row[2] / 2.0 for row in [r_bound, a_bound, z_bound]
            ]))
        self.register_buffer(
            'voxel_num',
            torch.LongTensor([(row[1] - row[0]) / row[2]
                              for row in [r_bound, a_bound, z_bound]]))
        ...
        if self.use_pa:
            self.pers_aggregation_net = self._configure_perspective_aggregation_net(
            )
    ...
    def _configure_perspective_net(self, depth_net_conf):
        return DepthNet(
            depth_net_conf['in_channels'],
            depth_net_conf['mid_channels'],
            self.output_channels,
    ...
    def _configure_perspective_aggregation_net(self):
        """build pixel cloud feature extractor"""
        return PersAggregation(self.output_channels, self.output_channels,
                                self.output_channels)
    ...
    def _forward_single_sweep(self,
        ...
        if self.use_pa:
            img_features = self.pers_aggregation_net(img_features.reshape(batch_size*num_cams, num_sweeps*self.output_channels, img_features.shape[3], img_features.shape[4]).to(torch.float32))
            img_features = img_features.reshape(batch_size*num_cams, self.output_channels, num_sweeps, img_features.shape[2], img_features.shape[3])
        ...
        geom_raz = geom_xyz.clone()
        x = geom_xyz[..., 0].clone()
        y = geom_xyz[..., 1].clone()
        geom_raz[..., 0] = torch.sqrt(x**2 + y**2) # r
        geom_raz[..., 1] = torch.atan2(y, x) # a
        
        geom_raz = ((geom_raz - (self.voxel_coord - self.voxel_size / 2.0)) / self.voxel_size).int()
        polar_feature_map = voxel_pooling(geom_raz, img_feat_with_depth.contiguous(),
                                    self.voxel_num.cuda())

        if self.xy_bev_head_with_deform_attn:
            geom_xyz = ((geom_xyz - (self.xy_voxel_coord - self.xy_voxel_size / 2.0)) / self.xy_voxel_size).int()
            bev_feature_map = voxel_pooling(geom_xyz, img_feat_with_depth.contiguous(),
                                        self.xy_voxel_num.cuda())
        else:
            bev_feature_map = None
            
        if is_return_depth:
            return polar_feature_map.contiguous(), depth, img_features.view(batch_size, num_cams, *img_features.shape[1:]).squeeze(3), bev_feature_map
        return polar_feature_map.contiguous(), bev_feature_map
    ...
    def forward(self,
        ...
        polar_key_frame_feature = key_frame_res[
            0] if is_return_depth else key_frame_res

        xy_key_frame_feature = key_frame_res[
            -1] if is_return_depth else key_frame_res

        polar_ret_feature_list = [polar_key_frame_feature]
        xy_ret_feature_list = [xy_key_frame_feature]
        for sweep_index in range(1, num_sweeps):
            with torch.no_grad():
                feature_map = self._forward_single_sweep(
                    ...
                    mats_dict,
                    is_return_depth=False)
                polar_ret_feature_list.append(feature_map[0])
                xy_ret_feature_list.append(feature_map[-1])
        if xy_ret_feature_list[0] is None:
            xy_bev = None
        else:
            xy_bev = torch.cat(xy_ret_feature_list, 1)
        if is_return_depth:
            return torch.cat(polar_ret_feature_list, 1), key_frame_res[1], key_frame_res[2], xy_bev
        else:
            return torch.cat(polar_ret_feature_list, 1), xy_bev
