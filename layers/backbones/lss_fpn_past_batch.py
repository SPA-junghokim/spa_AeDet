...
class DepthNet(nn.Module):
    ...
    def forward(self, x, mats_dict, num_sweeps=1):
        ...
        context_se = context_se.repeat(num_sweeps, 1, 1, 1)
...
class LSSFPN(nn.Module):
    ...
    def _forward_depth_net(self, feat, mats_dict, num_sweeps=1):
        return self.depth_net(feat, mats_dict, num_sweeps)
    ...
    def _forward_single_sweep(self,
        ...
        if self.depth_thresh:
            kept = (depth >= self.depth_thr).view(batch_size, num_cams, depth.shape[1], depth.shape[2], depth.shape[3])
    ...
    def _forward_past_sweep(self,
        ...
        batch_size, num_sweeps, num_cams, num_channels, img_height, \
            img_width = sweep_imgs.shape
        img_feats = self.get_cam_feats(sweep_imgs)
        source_features = img_feats.reshape(batch_size*num_sweeps, *img_feats.shape[2:])
        # source_features = img_feats[:, 0, ...]
        
        depth_features = []
        for i in range(num_sweeps):
            depth_feature = self._forward_depth_net(
                source_features[i*batch_size:(i+1)*batch_size].reshape(
                batch_size * num_cams,
                source_features.shape[2],
                source_features.shape[3],
                source_features.shape[4]),
        ...
        depth_feature = torch.vstack(depth_features)
        ...
        if self.use_cdn:
            ...
            # depth mapping
            image_scales = self.get_image_scale(mats_dict['intrin_mats'][:, 1:, ...].reshape(-1, *mats_dict['intrin_mats'].shape[2:]), mats_dict['ida_mats'][:, 1:, ...].reshape(-1, *mats_dict['ida_mats'].shape[2:]))
            offset_per_meter = self.depth_channels / self.d_bound[1] * self.min_focal_length / image_scales.view(-1, 1)
            offset = self.frustum[:, 0, 0, 2].view(1, -1) * offset_per_meter
            # print(f'offset min: {(offset[:, 1:] - offset[:, :-1]).min()}, max: {(offset[:, 1:] - offset[:, :-1]).max()}')  # todo
            real_depth_feature = self.depth_sampling(visual_depth_feature, offset)
        ...
        geom_xyz = self.get_geometry(mats_dict['sensor2ego_mats'][:, 1:, ...].reshape(-1, *mats_dict['sensor2ego_mats'].shape[2:]),
                                     mats_dict['intrin_mats'][:, 1:, ...].reshape(-1, *mats_dict['intrin_mats'].shape[2:]),
                                     mats_dict['ida_mats'][:, 1:, ...].reshape(-1, *mats_dict['ida_mats'].shape[2:]),
                                     mats_dict.get('bda_mat', None).repeat(num_sweeps,1,1))
        img_feat_with_depth = img_feat_with_depth.permute(0, 1, 3, 4, 5, 2)
        ...
        if self.depth_thresh:
            kept = (depth >= self.depth_thr).view(batch_size*num_sweeps, num_cams, depth.shape[1], depth.shape[2], depth.shape[3])
            geom_xyz, img_feat_with_depth = self.prepare_pooling(geom_xyz, img_feat_with_depth, kept, batch_size)
        ...
        if is_return_depth:
            return feature_map.contiguous(), depth, batch_size, num_sweeps
        return feature_map.contiguous(), batch_size, num_sweeps
    ...
    def forward(self,
        ...
        with torch.no_grad():
            self.eval()
            feature_map, batch_size, num_sweeps_past = self._forward_past_sweep(
                sweep_imgs[:, 1:, ...],
                mats_dict,
                is_return_depth=False)
            feature_map = feature_map.reshape(batch_size, num_sweeps_past, *feature_map.shape[1:]).permute(1,0,2,3,4)
            for sweep in range(num_sweeps_past):
                ret_feature_list.insert(0, feature_map[-1-sweep])
            self.train()
        if is_return_depth:
