...
class LSSFPN(nn.Module):
    def __init__(self, r_bound, a_bound, z_bound, d_bound, final_dim,
    ...
    def _forward_single_sweep(self,
        ...
        # change x to r & y to a
        # geom_xyz.shape = torch.Size([B, 6, 112, 16, 44, 3])
        #pdb.set_trace() 
        x = geom_xyz[..., 0].clone()
        y = geom_xyz[..., 1].clone()
        geom_xyz[..., 0] = torch.sqrt(x**2 + y**2) # r
        geom_xyz[..., 1] = torch.atan2(y, x) # a
        
        # self.voxel_coord = r, a, z의 처음 시작하는 위치를 뜻함
        # self.voxel_size = 0.8, math.pi/256, 8
        # [x, y, z]
        # [r, a] - [64, 256]
        ...
        import os
        import numpy as np
        numpy_image = feature_map.cpu()[0].detach().permute(1,2,0).numpy()
        fig = plt.figure()
        rand_id = np.random.randint(1000)
        for i in range(80):
            plt.imshow(numpy_image[:,:,i], cmap='viridis') 
            os.makedirs(f'visual_bev_polar/polar_bev', exist_ok=True)
            fig.savefig(f'visual_bev_polar/polar_bev/{i}.png')
        breakpoint()
