...
class DepthAggregation_prob(nn.Module):
    """
    pixel cloud feature extraction
    """
    def __init__(self, in_channels, mid_channels, out_channels):
        super(DepthAggregation_prob, self).__init__()
        ...
        kernel = torch.tensor([[[[0., 0., 0.], [0., 1., 0.], [0., 0., 0.]]]]).repeat(in_channels, out_channels, 1, 1)
        self.reduce_conv[0].weight.data = kernel
...
class LSSFPN(nn.Module):
    def __init__(self, depth_gt_training, x_bound, y_bound, z_bound, z_bound_pooling_consis, d_bound, final_dim,
                 ...
                 use_da_origin= False, use_da_prob=False, ):
        ...
        self.register_buffer(
            'voxel_size_pooling_consis',
            torch.Tensor([row[2] for row in [x_bound, y_bound, z_bound_pooling_consis]]))
        self.register_buffer(
            'voxel_coord_pooling_consis',
            torch.Tensor([
                row[0] + row[2] / 2.0 for row in [x_bound, y_bound, z_bound_pooling_consis]
            ]))
        self.register_buffer(
            'voxel_num_pooling_consis',
            torch.LongTensor([(row[1] - row[0]) / row[2]
                              for row in [x_bound, y_bound, z_bound_pooling_consis]]))
        ...
        self.use_da_origin = use_da_origin
        self.use_da_prob=use_da_prob
        
        if self.use_da_origin:
            self.depth_aggregation_net_origin = self._configure_depth_aggregation_net(
            )
        if self.use_da_prob:
            self.depth_aggregation_net_prob = self._configure_depth_aggregation_net_prob(
            )
    ...
    def _configure_depth_aggregation_net_prob(self):
        """build pixel cloud feature extractor"""
        return DepthAggregation_prob(int((self.d_bound[1]-self.d_bound[0])/self.d_bound[2]),
    int((self.d_bound[1]-self.d_bound[0])/self.d_bound[2]),
    int((self.d_bound[1]-self.d_bound[0])/self.d_bound[2]),)
    ...
    def _forward_voxel_net(self, img_feat_with_depth):
        if self.use_da_origin:
            ...
            img_feat_with_depth = (
                self.depth_aggregation_net_origin(img_feat_with_depth).view(
    ...
    def _forward_single_sweep(self,
        ...
        if self.use_da_prob:
            depth = self.depth_aggregation_net_prob(depth)
            depth = torch.softmax(depth, 1)
        img_features = depth_feature[:, self.depth_channels:(
                self.depth_channels + self.output_channels)].unsqueeze(2)
        
        if self.depth_gt_training:
            B_N, D, H, W = depth.shape
            depth = depth_labels.view(B_N, H, W, D).permute(0,3,1,2)
            reshaped_tensor = depth.view(-1, H, W)
            kernel_size = 5
            sigma = 1.0
            ...
            output_tensor = F.conv1d(reshaped_tensor, gaussian_kernel, padding=kernel_size//2, groups=112)
            output_tensor = output_tensor.view(B_N, D, H, W)
            depth_temp = depth.permute(0,2,3,1)
            temp = depth.argmax(1)
            breakpoint()
            
        img_feat_with_depth = depth.unsqueeze(1) * img_features
        ...
        geom_xyz_pooling_consis = ((geom_xyz - (self.voxel_coord_pooling_consis - self.voxel_size_pooling_consis / 2.0)) / self.voxel_size_pooling_consis).int()
        geom_xyz_pooling_consis[..., 0] = geom_xyz_pooling_consis[..., 0] + (geom_xyz_pooling_consis[..., 2] * self.voxel_num_pooling_consis[0])
        geom_xyz_pooling_consis[..., 2] = (geom_xyz_pooling_consis[..., 2] / self.voxel_size[2]).int()
        voxel_num_pooling_consis = self.voxel_num_pooling_consis.clone()
        voxel_num_pooling_consis[0] = voxel_num_pooling_consis[0] * voxel_num_pooling_consis[2]
        voxel_num_pooling_consis[2] = 1
        feature_map_pooling_consis = voxel_pooling(geom_xyz_pooling_consis, img_feat_with_depth.contiguous(), voxel_num_pooling_consis.cuda())
        ...
        if is_return_depth:
            return feature_map.contiguous(), depth, img_features.view(batch_size, num_cams, *img_features.shape[1:]).squeeze(3), feature_map_pooling_consis.contiguous()
    ...
    def forward(self,
        ...
        # from CenterTrack.src.lib.detector import Detector
        # from CenterTrack.src.lib.opts import opts

        # MODEL_PATH = "./CenterTrack/nuScenes_3Dtracking.pth"
        # TASK = 'tracking,ddd' # or 'tracking,multi_pose' for pose tracking and 'tracking,ddd' for monocular 3d tracking
        # DATASET = 'nuscenes'
        # HEAD_CONV = 256
        
        # opt = opts().init('{} --load_model {} --dataset {} --head_conv {}'.format(TASK, MODEL_PATH, DATASET, HEAD_CONV).split(' '))
        # tracking_detector = Detector(opt)
        
        # track_img = sweep_imgs[:, 0:1, ...][:,0,0] # [B, 3, 256, 704]
        # ret = tracking_detector.run(track_img.cpu())['results']
        ...
        if is_return_depth:
            return torch.cat(ret_feature_list, 1), key_frame_res[1], key_frame_res[2], key_frame_res[3]
        else:
