...
from mmdet3d.datasets.pipelines.custom import CustomDataset
...
custom_nusc_dataset = CustomDataset(
    data_root='data/nuScenes',
    ann_file = 'data/nuScenes/nuscenes_infos_train_mono3d.coco.json',
    #ann_file = 'data/nuScenes/nuscenes_infos_train_7split.pkl',
    pipeline = pipeline,
    )
...
class NuscMVDetDatasetwithPers(Dataset):
    def __init__(self,
                 ida_aug_conf,
                 bda_aug_conf,
                 classes,
    ...
    def __getitem__(self, idx):
        ...
        if self.is_train:
            ...
            pers_gt_boxes = custom_nusc_dataset.prepare_train_img(idx*6, cams) # chgd
        ...
        # ego center to camera center
        if self.align_camera_center:
            camera_center = sweep_sensor2ego_mats.mean(1, keepdim=True)[0, 0, :2, 3]
            center_mat = torch.eye(4)
            center_mat[:2, 3] = -camera_center
            if gt_boxes.shape[0] > 0:
                gt_boxes[:, :2] -= camera_center
            img_metas['camera_center'] = camera_center

        # unify the velocity prediction for the frames with different time intervals
        if len(self.key_idxes) == 2:
            time_interval = (cam_infos[0]['CAM_FRONT']['timestamp'] - cam_infos[1]['CAM_FRONT']['timestamp']) / 1000000.0
            interval_scale = torch.Tensor([time_interval * 2.])
            if gt_boxes.shape[0] > 0:
                gt_boxes[:, -2:] *= interval_scale
            img_metas['interval_scale'] = interval_scale
        ...
        if self.align_camera_center:
            bda_mat = bda_mat.matmul(center_mat)
...
def collate_fn(data, is_return_depth=False):
    ...
    for iter_data in data:
        ...
        pers_gt_boxes_batch.append(pers_gt_boxes)
    mats_dict = dict()
