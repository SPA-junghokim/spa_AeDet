...
class NuscMVDetDataset(Dataset):
    def __init__(self,
        ...
        if self.use_4split:
            self.cat2id = {name: i for i, name in enumerate(self.classes)}
            self.sample_indices = self._get_4split_sample_indices()
    ...
    def _get_4split_sample_indices(self):
        ori_sample_idxs = [*range(len(self.infos))]
        sample_indices = ori_sample_idxs[::4]
    ...
    def __len__(self):
        if self.use_cbgs or self.use_4split:
