...
#from .anchor_free_mono3d_head_centeronly import AnchorFreeMono3DHeadCenter # chgd
...
class FCOSMono3DHead(AnchorFreeMono3DHead):
    ...
    def forward_single(self, x, scale, stride):
        ...
        bbox_pred = super().forward_single(x)
        return bbox_pred
    ...
    @force_fp32(
        apply_to=('cls_scores', 'bbox_preds', 'dir_cls_preds', 'attr_preds',
                  'centernesses'))
    # def loss(self,
    #          cls_scores, # list of len 5, [B, 10, 116, 200] for index zero
    #          bbox_preds,
    #          dir_cls_preds,
    #          attr_preds,
    #          centernesses, # until five preds
    #          gt_bboxes, # [n, 4] len=batch
    #          gt_labels, # [n]
    #          gt_bboxes_3d, # CameraInstance3DBoxes n,9
    #          gt_labels_3d, #[n]
    #          centers2d, #[n,2]
    #          depths,  #[n]
    #          attr_labels, # [n]
    ...
    #         flatten_cls_scores,
    #         flatten_labels_3d.type(torch.int64), # chgd
    ...
    #             pos_bbox_preds[:, :2], # [66,2]
    #             pos_bbox_targets_3d[:, :2], # [66,2]
    #             weight=bbox_weights[:, :2],
    #             avg_factor=equal_weights.sum()) # 66
    ...
    def loss(self,
        ...
        if dir_cls_preds[0] is not None:
            flatten_dir_cls_preds = [
                dir_cls_pred.permute(0, 2, 3, 1).reshape(-1, 2)
                for dir_cls_pred in dir_cls_preds
            ]
        ...
        pos_bbox_preds = flatten_bbox_preds[pos_inds]
        breakpoint()
        if self.use_direction_classifier: pos_dir_cls_preds = flatten_dir_cls_preds[pos_inds]
        pos_centerness = flatten_centerness[pos_inds]
